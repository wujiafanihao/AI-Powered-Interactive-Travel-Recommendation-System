...
<div align="center">
  ...
  <p><em>图 1：个性化推荐与场景化分类首页</em></p>

  <img src="./img/image copy.png" alt="AI 智能帮搜" width="800"/>
  <p><em>图 2：多轮 AI 对话与智能意图识别</em></p>

  <img src="./img/image copy 2.png" alt="AI 对话界面" width="800"/>
  <p><em>图 3：AI 智能对话界面 - 自然语言交互</em></p>

  <img src="./img/image copy 3.png" alt="景点推荐列表" width="800"/>
  <p><em>图 4：景点推荐列表与筛选功能</em></p>

  <img src="./img/image copy 4.png" alt="景点详情展示" width="800"/>
  <p><em>图 5：景点详情展示页面</em></p>

  <img src="./img/image copy 5.png" alt="混合推荐算法展示" width="800"/>
  <p><em>图 6：混合推荐算法效果展示</em></p>

  <img src="./img/image copy 6.png" alt="用户画像与推荐" width="800"/>
  <p><em>图 7：用户画像与个性化推荐</em></p>

  <img src="./img/989f81b4cf1db33e0e322823ae3e609b.png" alt="景点搜索" width="800"/>
  <p><em>图 8：景点搜索与多维条件筛选</em></p>

  <img src="./img/bad12a0b4f1b608185a2a3a0e8dfa1de.png" alt="景点详情与 AI 导游" width="800"/>
  <p><em>图 9：景点详情页、真实用户评论与专属 AI 导游抽屉</em></p>
</div>
---
...
| **AI 智能对话** | 融合高精度混合算法与大语言模型，支持多轮聊天和历史记录（自动保存景点卡片数据） | ✅ |
| **专属 AI 导游** | 景点详情页内置专属 AI 导游，提供沉浸式体验 | ✅ |
| **RAG 知识摘要** | 以混合推荐搜索出的精确景点信息为上下文，喂给大模型提取总结亮点，消除幻觉 | ✅ |
| **智能帮搜** | 自然语言→结构化条件→调起三路混合推荐引擎重打分（高精度）→生成推荐卡片并透传混合得分 | ✅ |
| **意图识别** | 规则+LLM双层判断（搜索/问答/帮助/闲聊） | ✅ |
| **前端界面** | 首页推荐、景点列表、景点详情、AI对话（卡片显示混合得分）、我的收藏、登录/注册 | ✅ |
...
意图识别器（规则 + LLM 双层判断）
    ...
    │       ├── 调起混合推荐引擎 → 结合历史行为计算协同/内容基础分，加持条件重排行
    │       ├── 组装卡片与文本 → 将选出的高分精准景点透传混合分 (🔥混合得分)
    │       └── 喂入 RAG 生成管线 → LLM 提炼景点亮点 → 返回带卡片的拟人化回复
    │
    ├── 问答意图 → RAG 管线
    │       │
...
### 3. 智能帮搜与高精度 RAG 赋能（intent_recognizer.py → SmartSearcher）

```
"秋天带老人去南京看枫叶"
    → LLM 提取条件 → {"city":"南京", "season":"秋", "target_group":"老年", "keywords":["枫叶"]}
    → 将条件传入混合引擎 → [基础推荐分数 + 5000条件分 + ...] 综合运算
    → 降级策略保障 → 如果服务异常则退回到 SQL DB 查询
    → 组装 RAG Context → 将提取出的高分景点信息组合成上下文
    → 调用 DeepSeek 生成摘要式推荐回答
```
